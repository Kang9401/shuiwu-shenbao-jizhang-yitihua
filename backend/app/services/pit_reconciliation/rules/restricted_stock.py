...
def restricted_stock_details(rows, declarations):
    business=defaultdict(lambda: Decimal("0.00")); declared=defaultdict(lambda: Decimal("0.00")); ids=defaultdict(set)
    for row in rows: business[(row.org_code,row.customer_name)]+=abs(row.withheld_tax or Decimal("0.00")); ids[(row.org_code,row.customer_name)].add(row.id_number)
