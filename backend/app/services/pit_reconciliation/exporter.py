...
def build_pit_workpaper_sheets(db, company_id: int, period_id: int, workpaper) -> OrderedDict[str, pd.DataFrame]:
    ...
    a1_columns = ["机构代码", "营业部名称", "期间", "姓名", "申报税额", "工资表税额", "差异", "差异原因"]
    ...
    sheets = OrderedDict({
        ...
        "附A1 工资薪金等个税差异": _frame([{"机构代码": row.get("org_code"), "营业部名称": row.get("org_name"), "期间": tax_period, "姓名": row.get("person_or_customer_name"), "申报税额": row.get("target_amount"), "工资表税额": row.get("source_amount"), "差异": row.get("difference"), "差异原因": row.get("manual_reason") or row.get("auto_reason")} for row in detail_rows("salary_tax", [])], a1_columns),
