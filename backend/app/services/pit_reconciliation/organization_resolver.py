...
class PitOrganizationResolver:
    def __init__(self, db, company_id: int):
        ...
        self.bank_account_to_code = {
            str(row.bank_subaccount).strip(): row.org_code.strip()
            for row in rows
            if row.bank_subaccount and str(row.bank_subaccount).strip().lower() not in {"none", "nan"}
        }
    ...
    def resolve_by_bank_account(self, value: str | None): return self.get_org(self.bank_account_to_code.get((value or "").strip()))
